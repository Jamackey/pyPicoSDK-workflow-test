...
class StreamingScope:
    ...
    def config_streaming(
        ...
        if max_buffer_size is None:
            ...
            # Main sample buffer
            self.buffer_array = np.zeros(shape=max_buffer_size)
        # Maximum size of buffer before overwritinG
    ...
    def run_streaming(self) -> None:
        ...
        # Setup initial buffer for streaming (clear all buffers first)
        self.scope.set_data_buffer(0, 0, action=ACTION.CLEAR_ALL)
        self.scope.set_data_buffer(self.channel, self.samples, segment=0,
                                   buffer=self.np_buffer[0])
        # start streaming
    ...
    def main_streaming_loop(self) -> None:
        ...
        if n_samples > 0:
            ...
            self.buffer_array = \
                np.concatenate([self.buffer_array] +
                               [self.np_buffer[self.buffer_index]
        ...
        if info['status'] == 407:
            # Switch between buffer segment index 0*samples and 1*samples
            self.buffer_index = (self.buffer_index + 1) % 2
            self.scope.set_data_buffer(
                self.channel,
                self.samples,
                segment=self.buffer_index,
                action=ACTION.ADD,
                buffer=self.np_buffer[self.buffer_index])
