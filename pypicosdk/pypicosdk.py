...
def export_to_csv(
    ...
    if '.csv' not in filename:
        filename += '.csv'
    if isinstance(channels_buffer[0], list):
        _export_to_csv_rapid(filename, channels_buffer, time_axis)
    elif isinstance(channels_buffer[0], np.array):
...
def convert_time_axis(
    ...
    Args:
        ...
        current_units (str): The starting time unit of
            the data (e.g., 's', 'ms', 'us').
        convert_units (str): The target time unit for
            the conversion (e.g., 'ns').
    ...
    """
    diff = (time_standard_form_m[convert_units] -
            time_standard_form_m[current_units])
    time_axis = np.multiply(time_axis, 10**diff)
    return time_axis, convert_units


def resolution_enhancement(
        buffer: np.ndarray,
        enhanced_bits: float,
        padded: bool = True
        ) -> np.ndarray:
    """
    Returns the buffer after applying a moving average filter with the
    ...
    if not 0.5 <= enhanced_bits <= 4:
        raise PicoSDKException(
            f"Invalid enhanced_bits value: {enhanced_bits}."
            f"Must be between 0.5 and 4.")

    window_size = int(4 ** enhanced_bits)
    return np.convolve(buffer, np.ones(window_size)/window_size,
                       mode=['valid', 'same'][padded])
