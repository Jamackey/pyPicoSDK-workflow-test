...
class PicoScopeBase():
    """PicoScope base class including common SDK and python modules and
    functions"""

    # Class Functions
    def __init__(self, dll_name, *args):
        # Pytest override
        ...
        if self._pytest:
            ...
            self.dll = ctypes.CDLL(
                os.path.join(_get_lib_path(), dll_name + ".dll")
            )
    ...
    def _get_attr_function(self, function_name: str) -> ctypes.CDLL:
        ...
        For example, `_get_attr_function("OpenUnit")` will return
            `self.dll.ps####aOpenUnit()`.
    ...
    def _error_handler(self, status: int) -> None:
        ...
        if status != 0:
            if status in [POWER_SOURCE.SUPPLY_NOT_CONNECTED]:
                warnings.warn(
                    "Power supply not connected.", PowerSupplyWarning
                )
                return
    ...
    def open_unit(
        self, serial_number: int = None, resolution: RESOLUTION = 0
    ) -> None:
        """
        ...
        self._call_attr_function(
            "OpenUnit", ctypes.byref(self.handle), serial_number, resolution
        )
    ...
    def close_unit(self) -> None:
        ...
        This calls the PicoSDK `CloseUnit` function to properly disconnect
        from the device.
    ...
    def stop(self) -> None:
        ...
        Returns:
            None
        """
        self._call_attr_function("Stop", self.handle)

    def is_ready(self) -> None:
        """
        Blocks execution until the PicoScope device is ready.

        Continuously calls the PicoSDK `IsReady` function in a loop,
        checking if the device is prepared to proceed with data acquisition.
        ...
        while True:
            self._call_attr_function(
                "IsReady", self.handle, ctypes.byref(ready)
            )
            if ready.value != 0:
                break
    ...
    def check_for_update(self, n_infos: int = 8) -> tuple[list, bool]:
        ...
        return (
            list(info_array)[: n_returned.value],
            bool(updates_required.value),
        )
    ...
    def get_unit_info(self, unit_info: UNIT_INFO) -> str:
        ...
        Args:
            unit_info (UNIT_INFO): Specify information from PicoScope unit
                i.e. UNIT_INFO.PICO_BATCH_AND_SERIAL.
    ...
    def get_nearest_sampling_interval(self, interval_s: float) -> dict:
        ...
        requested sample interval. It does not change the configuration
        of the oscilloscope.
        ...
        Args:
            interval_s (float): Time value in seconds (s) you would like to
                obtain.
        ...
        return {
            ...
            "actual_sample_interval": time_interval.value,
        }

    def get_timebase(
        self, timebase: int, samples: int, segment: int = 0
    ) -> dict:
        """
        ...
        Args:
                timebase (int): Selected timebase multiplier (refer to
                    programmer's guide).
                samples (int): Number of samples.
                segment (int, optional): The index of the memory segment to
                    use.
        ...
        return {
            "Interval(ns)": time_interval_ns.value,
    ...
    def sample_rate_to_timebase(
        self, sample_rate: float, unit: SAMPLE_RATE = SAMPLE_RATE.MSPS
    ):
        """
        ...
        Use `get_nearest_sample_interval(interval_s)` to get the full timebase
        and actual interval achieved.
    ...
    def interval_to_timebase(
        self, interval: float, unit: TIME_UNIT = TIME_UNIT.S
    ):
        """
    ...
    def _get_maximum_adc_value(self) -> int:
        ...
        self._call_attr_function(
            "MaximumValue", self.handle, ctypes.byref(max_value)
        )
        return max_value.value

    def get_time_axis(
        self, timebase: int, samples: int, pre_trig_percent: int = None
    ) -> list:
        """
        ...
        Args:
            ...
            pre_trig_percent: Percent to offset the 0 point by.
    ...
    def get_trigger_time_offset(
        self, time_unit: TIME_UNIT, segment_index: int = 0
    ) -> int:
        """
    ...
    def get_values_trigger_time_offset_bulk(
        ...
        Returns:
            list[tuple[int, PICO_TIME_UNIT]]: ``[(offset, unit), ...]`` for
            each segment beginning with ``from_segment_index``.
        """
    ...
    def get_values_overlapped(
        ...
        """Retrieve overlapped data from multiple segments for block or rapid
            block mode.

        Args:
            start_index: Index within the circular buffer to begin reading
                from.
    ...
    def get_device_resolution(self) -> RESOLUTION:
        ...
        adc_limits = self.get_adc_limits()  # pylint: disable=no-member
        self.min_adc_value, self.max_adc_value = adc_limits
    ...
    def mv_to_adc(
        ...
        Args:
                ...
                channel_range (int): Range of channel in millivolts
                    i.e. 500mV.
                channel (CHANNEL, optional): Channel associated with ``mv``.
                    The probe scaling for the channel will be applied if
                    provided.
    ...
    def adc_to_mv(
        ...
        return (
            (float(adc) / float(self.max_adc_value)) * channel_range_mv
        ) * scale

    def buffer_adc_to_mv(self, buffer: list, channel: str) -> list:
        """Converts an ADC buffer list to mV list"""
        return [
            self.adc_to_mv(sample, self.range[channel], channel)
    ...
    def channels_buffer_adc_to_mv(self, channels_buffer: dict) -> dict:
        ...
        for channel in channels_buffer:
            ...
            channels_buffer[channel] = (
                (data / self.max_adc_value) * channel_range_mv
            ) * channel_scale
    ...
    def channels_buffer_ctype_to_list(self, channels_buffer):
        """Takes a ctypes channel dictionary buffer and converts into a integer
            array."""
        for channel in channels_buffer:
    ...
    def _thr_hyst_mv_to_adc(
        ...
        if channel in self.range:
            upper_adc = self.mv_to_adc(
                threshold_upper_mv, self.range[channel], channel
            )
            lower_adc = self.mv_to_adc(
                threshold_lower_mv, self.range[channel], channel
            )
            hyst_upper_adc = self.mv_to_adc(
                hysteresis_upper_mv, self.range[channel], channel
            )
            hyst_lower_adc = self.mv_to_adc(
                hysteresis_lower_mv, self.range[channel], channel
            )
        else:
    ...
    def _change_power_source(self, state: POWER_SOURCE) -> 0:
        ...
        Args:
                state (POWER_SOURCE): Power source variable
                    (i.e. POWER_SOURCE.SUPPLY_NOT_CONNECTED).
        """
        self._call_attr_function("ChangePowerSource", self.handle, state)
    ...
    def set_all_channels_off(self):
        ...
        for channel in range(int(channels)):
            self.set_channel(channel,  # pylint: disable=no-member
                             enabled=False)
    ...
    def set_simple_trigger(
        ...
        Args:
            ...
            direction (TRIGGER_DIR | str, optional):
                Trigger direction (e.g., ``TRIGGER_DIR.RISING``).
            delay (int, optional):
                Delay in samples after the trigger condition is met before
                starting capture.
            auto_trigger (int, optional): Timeout in **microseconds** after
                which data capture proceeds even if no trigger occurs.
                If 0, the PicoScope will wait indefintely.
        ...
        if channel in self.range:
            threshold_adc = self.mv_to_adc(
                threshold_mv, self.range[channel], channel
            )
        else:
    ...
    def set_trigger_channel_conditions(
        ...
        Args:
            conditions (list[tuple[CHANNEL, TRIGGER_STATE]]):
                A list of tuples describing the CHANNEL and TRIGGER_STATE for
                that channel
            action (int, optional): Action to apply this condition relateive
                to any previous condition.
    ...
    def set_trigger_channel_directions(
        ...
        If multiple directions are needed, channel, direction and
        threshold_mode can be given a list of values.
        ...
        if isinstance(channel, list):
            ...
            for i in range(dir_len):
                dir_struct[i] = PICO_DIRECTION(
                    channel[i], direction[i], threshold_mode[i]
                )
        else:
            dir_len = 1
    ...
    def set_advanced_trigger(
        ...
        upper_adc, lower_adc, hyst_upper_adc, hyst_lower_adc = (
            self._thr_hyst_mv_to_adc(
                channel,
                threshold_upper_mv,
                threshold_lower_mv,
                hysteresis_upper_mv,
    ...
    def set_pulse_width_trigger(
        ...
        """
        Configures a pulse width trigger using a specified channel and timing
        parameters.

        This method sets up a trigger condition where a pulse on the specified
        channel must be within or outside a defined pulse width window.
        The trigger logic uses both level thresholds and pulse width
        qualifiers to define the trigger behavior.
        ...
        Args:
            ...
            time_upper_units (TIME_UNIT, optional):
                Units for `time_upper`. Default is microseconds.
            time_lower (float, optional):
                Lower time bound for pulse width. Default is 0 (disabled).
            time_lower_units (TIME_UNIT, optional):
                Units for `time_lower`. Default is microseconds.
            threshold_upper_mv (float, optional):
                Upper voltage threshold in millivolts. Default is 0.0 mV.
            threshold_lower_mv (float, optional):
                Lower voltage threshold in millivolts. Default is 0.0 mV.
            hysteresis_upper_mv (float, optional):
                Hysteresis for upper threshold in mV. Default is 0.0 mV.
            hysteresis_lower_mv (float, optional):
                Hysteresis for lower threshold in mV. Default is 0.0 mV.
            trig_dir (THRESHOLD_DIRECTION, optional):
                Trigger direction for the initial pulse.
                If None, inferred as opposite of `direction`. Default is None.
            threshold_mode (THRESHOLD_MODE, optional):
                Specifies whether thresholds are in level or window mode.
                Default is LEVEL.
            auto_trigger_us (int, optional): Time in microseconds after which
                an automatic trigger occurs. Default is 0 (disabled).
        """

        # If no times are set, raise an error.
        if time_upper == 0 and time_lower == 0:
            raise PicoSDKException(
                "No time_upper or time_lower bounds specified for Pulse Width "
                "Trigger"
            )
        ...
        # If no trigger direction is specified, use the oppsite direction,
        # otherwise raise an error
        if trig_dir is None:
            if direction is THRESHOLD_DIRECTION.RISING:
                trig_dir = THRESHOLD_DIRECTION.FALLING
            elif direction is THRESHOLD_DIRECTION.FALLING:
                trig_dir = THRESHOLD_DIRECTION.RISING
            else:
                raise PicoSDKException(
        ...
        self.set_trigger_channel_directions(
            channel=channel, direction=trig_dir, threshold_mode=threshold_mode
        )
        ...
        self.set_trigger_channel_properties(
            threshold_upper=upper_adc,
            hysteresis_upper=hyst_upper_adc,
            threshold_lower=lower_adc,
            hysteresis_lower=hyst_lower_adc,
            channel=channel,
        ...
        self.set_pulse_width_qualifier_directions(  # pylint: disable=E1101
            channel=channel,
            direction=direction,
    ...
    def set_data_buffer_for_enabled_channels(
        ...
        Args:
            ...
            captures: If larger than 0, it will create multiple buffers for
                RAPID mode.

        Returns:
            dict: A dictionary mapping each channel to its associated data
                buffer.
        """
        # Clear the buffer
        ...
        if captures > 0:
            for channel in self.range:
                np_buffer = self.set_data_buffer_rapid_capture(
                    channel,
                    samples,
                    captures,
                    segment,
        ...
        # Single
            ...
            for channel in self.range:
                channels_buffer[channel] = self.set_data_buffer(
                    channel,
                    samples,
                    segment,
                    datatype,
    ...
    def set_data_buffer(
        ...
        """
        Allocates and assigns a data buffer for a specified channel.

        Args:
            channel (int): The channel to associate the buffer with
                (e.g., CHANNEL.A).
            samples (int): Number of samples to allocate in the buffer.
            segment (int, optional): Memory segment to use.
            datatype (DATA_TYPE, optional): C data type for the buffer
                (e.g., INT16_T).
            ratio_mode (RATIO_MODE, optional): Downsampling mode.
            action (ACTION, optional): Action to apply to the data buffer
                (e.g., CLEAR_ALL | ADD).
            buffer (np.ndarray | None, optional): Send a preallocated numpy
                buffer to be populated.
                If left as None, this function creates its own buffer.

        Returns:
            np.array | None: The allocated buffer or ``None`` when clearing
                existing buffers.
    ...
    def set_data_buffer_rapid_capture(
        ...
        """
        Allocates and assigns multiple data buffers for rapid block capture on
        a specified channel.
    ...
    def set_data_buffers(
        ...
        Args:
            ...
            ratio_mode (RATIO_MODE, optional): Downsampling mode.
            action (ACTION, optional): Action to apply to the data buffer.
            buffers (np.ndarray | None, optional): Send preallocated 2D numpy
                buffers to be populated. Min buffer first, followed by max
                buffer. If left as None, this function creates its own buffers.
    ...
    def set_data_buffers_rapid_capture(
        ...
        """
        Allocate and assign max and min NumPy-backed data buffers for rapid
        block capture on a specified channel.
    ...
    def run_simple_block_capture(
        ...
        Args:
            ...
            conv_to_mv: If True, function will return a float mV array.
                If False, function will return a ADC array specified by
                datatype arg.
        ...
        Returns:
            tuple[dict, list]: Dictionary of channel buffers (in mV) and the
            time axis in nano-seconds (numpy array).

        Examples:
            >>> scope.set_channel(CHANNEL.A, RANGE.V1)
            >>> scope.set_simple_trigger(CHANNEL.A, threshold_mv=500)
            >>> buffers = scope.run_simple_block_capture(timebase=3,
                                                         samples=1000)
        """

        # Create data buffers.
        # If Ratio Mode is TRIGGER, create a trigger buffer
        ...
        actual_samples = self.get_values(
            samples, start_index, segment, ratio, ratio_mode
        )
    ...
    def run_simple_rapid_block_capture(
        ...
        Returns:
            tuple[dict, np.ndarray]: Dictionary of channel buffers (in mV) and
            the time axis in nano-seconds (numpy array).
        """
    ...
    def run_block_capture(
        self, timebase, samples, pre_trig_percent=50, segment=0
    ) -> int:
        """
        Runs a block capture using the specified timebase and number of
        samples.

        This sets up the PicoScope to begin collecting a block of data,
        divided into pre-trigger and post-trigger samples. It uses the PicoSDK
        `RunBlock` function.

        Args:
                timebase (int): Timebase value determining sample interval
                    (refer to PicoSDK guide).
                samples (int): Total number of samples to capture.
                pre_trig_percent (int, optional): Percentage of samples to
                    capture before the trigger.
                segment (int, optional): Memory segment index to use.

        Returns:
                int: Estimated time (in milliseconds) the device will be busy
                    capturing data.
        """
    ...
    def run_streaming(
        ...
        Args:
            ...
            max_post_trigger_samples: Number of post-trigger samples to
                collect.
    ...
    def get_enumerated_units(self) -> tuple[int, str, int]:
        """
        Returns count, serials and serial string length of a specific
        PicoScope unit.
    ...
    def get_values(
        ...
        This function should be called after confirming the device is ready
        using `is_ready()`. It invokes the underlying PicoSDK `GetValues`
    ...
    def get_streaming_latest_values(self, channel, ratio_mode, data_type):
    ...
    def is_over_range(self) -> list:
        ...
        over_range_channels = [
            CHANNEL_NAMES[i] for i in range(8) if self.over_range & (1 << i)
        ]

        if over_range_channels:
            warnings.warn(
                f"Overrange detected on channels: {
                    ', '.join(over_range_channels)}.",
                OverrangeWarning,
            )
